---
title: Webhooks
description: "Receive real-time notifications when events occur in your ByteSend account"
---

## Overview

Webhooks allow you to receive HTTP POST requests to your server when events occur in ByteSend, such as when an email is delivered, bounced, or clicked. This enables you to build real-time integrations and automate workflows.
...
<Steps>
  ...
  <Step title="Add webhook in dashboard">
    Go to [Webhooks](https://bytesend.cloud/webhooks) in your ByteSend dashboard and create a new webhook:
  ...
  <Step title="Verify webhook signatures">
    Always verify webhook signatures to ensure requests are from ByteSend. See the [Signature Verification](#signature-verification) section below.
  </Step>
</Steps>
...
| Header                | Description                            |
| --------------------- | -------------------------------------- |
| `X-ByteSend-Signature` | HMAC-SHA256 signature for verification |
| `X-ByteSend-Timestamp` | Unix timestamp in milliseconds         |
| `X-ByteSend-Event`     | Event type                             |
| `X-ByteSend-Call`      | Unique webhook call ID                 |
| `X-ByteSend-Retry`     | `true` if this is a retry attempt      |
...
const bytesend = new ByteSend("bs_your_api_key");
...
import { Webhooks } from "bytesend-js";
...
// Usage
const signature = request.headers.get("X-ByteSend-Signature");
const timestamp = request.headers.get("X-ByteSend-Timestamp");
...
If your endpoint doesn't return a 2xx response, ByteSend will retry delivery with exponential backoff:
...
<AccordionGroup>
  ...
  <Accordion title="Verify signatures">
    Always verify the `X-ByteSend-Signature` header to ensure requests are from
    ByteSend and haven't been tampered with.
  </Accordion>
...
1. Go to [Webhooks](https://bytesend.cloud/webhooks)
