...
1. Go to [Contacts](https://bytesend.cloud/contacts) and select a contact book
...
```bash
curl -X PATCH https://bytesend.cloud/api/v1/contactBooks/{contactBookId} \
  -H "Authorization: Bearer bs_your_api_key" \
  -H "Content-Type: application/json" \
  -d '{
    "doubleOptInEnabled": true
...
The confirmation email body can be customized using the ByteSend email editor (via the dashboard) or by providing the editor JSON content via the API.
