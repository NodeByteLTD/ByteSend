...
1. Go to [Contacts](https://app.bytesend.cloud/contacts)
...
```bash
curl -X POST https://app.bytesend.cloud/api/v1/contactBooks \
...
```bash
curl -X POST https://app.bytesend.cloud/api/v1/contactBooks/{contactBookId}/contacts \
  -H "Authorization: Bearer us_your_api_key" \
  -H "Content-Type: application/json" \
  -d '{
    "email": "jane@example.com",
...
```bash
curl -X POST https://app.bytesend.cloud/api/v1/campaigns \
  -H "Authorization: Bearer us_your_api_key" \
  -H "Content-Type: application/json" \
  -d '{
