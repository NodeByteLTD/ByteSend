...
<Steps>
  ...
  <Step title="Send the campaign">
    ByteSend replaces each variable with that contact's value when the campaign
    is rendered.
  </Step>
</Steps>
...
```bash
curl -X POST https://bytesend.cloud/api/v1/contactBooks/{contactBookId}/contacts \
  -H "Authorization: Bearer bs_your_api_key" \
  -H "Content-Type: application/json" \
  -d '{
    "email": "jane@example.com",
