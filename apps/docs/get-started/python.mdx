---
title: Python SDK
description: Official ByteSend Python SDK for sending emails and managing contacts.
icon: python
---

This guide shows how to install and use the official `bytesend` Python SDK.
...
client = ByteSend("bs_xxx")

# Option B: custom base URL (self-hosted)
client = ByteSend("bs_xxx", url="https://your-domain.example")
```
...
ByteSend forwards your custom headers to SES. Only the `X-ByteSend-Email-ID` and `References` headers are managed automatically.
...
By default the client raises `ByteSendHTTPError` for non-2xx responses. To handle errors as return values, pass `raise_on_error=False`.
...
except ByteSendHTTPError as e:
    print("request failed:", e)

# Returns (None, error) instead of raising
client = ByteSend("bs_xxx", raise_on_error=False)
