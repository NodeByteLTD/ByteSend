...
The ByteSend codebase is available on GitHub.
...
- `marketing`: The code for the landing page of ByteSend
...
- `sdk` package contains TypeScript SDK for ByteSend REST API
...
## Running ByteSend locally

To run ByteSend locally, you will need to setup the following:
...
<Steps>
  <Step title="Fork the repo">
    Click on the fork button on [GitHub](https://github.com/NodeByteHosting/bytesend) to fork the repo
  </Step>
...
Next, [create a new GitHub App](https://github.com/settings/applications/new). This will allow you to sign in to ByteSend with your GitHub account.
...
Next, we need to add in the [AWS credentials](https://docs.bytesend.cloud/get-started/create-aws-credentials). Follow the detailed guide to get the AWS credentials with accurate permissions and add them in:
