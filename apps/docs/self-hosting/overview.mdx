...
ByteSend depends on AWS SES to send emails and SNS to receive email status. Along with that it also depends on Postgres as a database and Redis for queue. Copy the `.env.selfhost.example` file to `.env` and fill in the values.
...
<Tip>
  ...
  ByteSend uses GitHub authentication for login.
...
Callback URL : `https://<your-bytesend-instance>/api/auth/callback/github`
...
```
  ...
  ByteSend uses Postgres as a database and Redis as a queue. You need to create a new database and add the following environment variables.
...
If you're sharing a Redis instance across multiple apps and using Redis ACL for isolation, set `REDIS_KEY_PREFIX` to namespace all keys (e.g. `REDIS_KEY_PREFIX="bytesend"` prefixes all keys with `bytesend:`).
...
```env
  NEXTAUTH_URL="https://<your-bytesend-instance>"
...
You can use any platform that supports Docker to host ByteSend. If you have any questions drop in the [discord channel](https://discord.gg/gbsvjb9MqV) and we'll try to help you out.
...
In order to send emails, you need to select a region in AWS. Use a region where your users are located / where ByteSend is hosted. If you're confused just use `us-east-1`.

You can check available regions [here](https://docs.aws.amazon.com/general/latest/gr/ses.html)

Once you log in to ByteSend, it will prompt you add SES configuration.
...
The SMTP proxy server is an optional component that allows applications to send emails through ByteSend using standard SMTP protocol instead of the REST API. This is useful for legacy applications, email clients, or any software that needs to send emails via SMTP.
...
```yaml
name: bytesend-smtp-server

services:
  smtp-server:
    container_name: bytesend-smtp-server
    image: bytesend/smtp-proxy:latest
    environment:
      SMTP_AUTH_USERNAME: "bytesend" # Username for SMTP authentication
      BYTESEND_BASE_URL: "https://your-bytesend-instance.com" # Your ByteSend instance URL

      # Optional: SSL certificate paths for secure connections
      # BYTESEND_API_KEY_PATH: "/certs/server.key"
      # BYTESEND_API_CERT_PATH: "/certs/server.crt"
...
- **Username**: `bytesend` (or your custom `SMTP_AUTH_USERNAME`)
- **Password**: Your ByteSend API key
- **Encryption**: STARTTLS (ports 25, 587, 2587) or SSL/TLS (ports 465, 2465)

<Tip>
  The SMTP proxy forwards all emails to your ByteSend instance, so make sure your
  main ByteSend application is running and accessible.
</Tip>
...
<Tip>
  A community member shared a short write-up on hosting ByteSend with
